...
Tools.prototype = {
  ...
  getVarLine: function (regx, text) {
    ...
    if (!ls || !Array.isArray(ls)) return null;
  ...
  getVars: function (texts) {
    ...
    for (var i = 0, len = texts.length; i < len; ++i) {
      ...
      if (one[0] && ms && Array.isArray(ms) && ms.length) {
        keys.push(ms[1]);
  ...
  //替换标签上属性内容
  extractTagAttr: function (globals, htmlContent, text, attrReg) {
    ...
    keys = [].concat(Object.keys(globals), keys);
    vals = [].concat(Object.values(globals), vals);
  ...
  //替换标签内容属性
  extractTagContent: function (globals, htmlContent, regx, text) {
    //获取标签之间的属性内容
